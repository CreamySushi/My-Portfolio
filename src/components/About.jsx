...
const About = () => {
  const educations = [
    ...
    },
    {
      id: 2,
      title: "Senior High",
      school: "Biñan City Senior High School",
      year: "2022",
    },
    {
      id: 3,
      title: "Junior High",
      school: "Jacobo National High School",
      year: "2020",
    },
    {
      id: 4,
      title: "Primary",
      school: "Malaban Elementary School",
      year: "2016",
    },
  ];
  ...
  const experiences = [
    {
      ...
      company: "Wally Wombats",
      location: "Las Vegas, Nevada, US",
      year: "May 2025 – Sept 2025",
      type: "OJT",
      descrition: [
        "Develop a 3D mobile game through Unity that cater children and design 3D model and animation in Blender",
        "Manage builds with Git and led playtesting and bug fixes",
        "Published in AppStore and Google Play Store",
      ],
    },
  ];
  ...
  const certifications = [
    {
      id: 1,
      type: "CAD ESSENTIALS WITH AUTODESK FUSION",
  ...
  const seminars = [
    {
      id: 1,
      type: "Unlock the Power of the Cloud: Introduction to AWS & Career Opportunities",
      issuer: "AWS Learning Club UPHS",
      year: "May 04, 2024",
    },
  ];
  ...
  return (
    <div
      id="About"
      className="min-h-screen pb-20 bg-(--bg-primary) dark:bg-(--dark-bg-primary)"
    >
      <div className="pt-16 text-center justify-center px-6 w-full md:px-16 lg:px-24 xl:px-40">
        <h1 className="relative inline-block mx-auto text-(--text-primary) dark:text-(--dark-text-primary) text-3xl font-bold uppercase">
          About My
          <span className="absolute left-0 -bottom-1 h-0.75 w-full rounded-full bg-(--accent-primary)" />
        </h1>
            ...
            <div>
              {educations.map((educ) => (
                <div className="flex items-start justify-between gap-4 rounded-2xl mb-6 px-6 py-4 bg-(--bg-tertiary) dark:bg-(--accent-primary)/10">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-(--accent-pressed) dark:text-(--accent-secondary) mb-0.5">
                      {educ.title}
                    </p>
                    <p className="text-sm font-medium text-(--text-primary) dark:text-(--dark-text-primary) leading-snug">
                      {educ.school}
                    </p>
                  </div>
                  <span className="text-xs text-(--text-tertiary) shrink-0 mt-0.5 font-mono">
                    {educ.year}
                  </span>
                </div>
              ))}
            ...
            <div className="shadow-xl rounded-2xl px-6 py-6 dark:bg-zinc-900 border border-(--border) dark:border-(--dark-border)">
              ...
              <div>
                {certifications.map((cert) => (
                  <div className="flex items-start justify-between gap-4 rounded-2xl mb-6 px-6 py-4 bg-(--bg-tertiary) dark:bg-(--accent-primary)/10">
                    <div>
                      <p className="text-sm font-medium uppercase  text-(--accent-pressed) dark:text-(--accent-secondary) leading-snug mb-0.5">
                        {cert.type}
                      </p>
                      <p className="text-xs font-semibold text-(--text-primary) dark:text-(--dark-text-primary) tracking-wide">
                        {cert.issuer}
                      </p>
                    </div>
                    <span className="text-xs text-(--text-tertiary) shrink-0 mt-0.5 font-mono">
